...
$table = <<<EOT
 (
    ...
    FROM 
    wdevoluciones 
    WHERE cod_audi::text LIKE '%$cod_audi%' AND cod_usua = $cod_usua group by 1,2,3,4,5) AS tabla) AS totalrows
    FROM 
    wdevoluciones 
    WHERE cod_audi::text LIKE '%$cod_audi%' AND cod_usua = $cod_usua group by 1,2,3,4,5
